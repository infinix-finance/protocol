...
describe.only("ClearingHouse - open/close position Test", () => {
  ...
  describe("position", () => {
    ...
    it("open position - two longs", async () => {
      ...
      expect(pos.size.d).to.eq("54545454545454545454");
      expect(pos.margin.d).to.eq(toFullDigitStr(120));

      const margin = await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
        quoteToken.address,
        alice.address
      );
      expect(margin.d).to.eq(toFullDigitStr(120));
    });
    ...
    it("open position - two shorts", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(40), toDecimal(5), toDecimal(25));

      // create position 2
      // AMM after: 600 : 166.6666666667
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(40), toDecimal(5), toDecimal(41.67));
      ...
      expect(pos2.size.d).to.eq("-66666666666666666667");
      expect(pos2.margin.d).to.eq(toFullDigitStr(80));

      const margin = await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
        quoteToken.address,
        alice.address
      );
      expect(margin.d).to.eq(toFullDigitStr(80));
    });
    ...
    it("open position - two equal size but opposite side positions", async () => {
      ...
      expect(await quoteToken.balanceOf(alice.address)).to.eq(
        toFullDigit(4940, +(await quoteToken.decimals()))
      );
      ...
      const ret = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(300), toDecimal(2), toDecimal(37.5));

      const pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq(0);
      expect(pos.margin.d).to.eq(toFullDigit(0));

      const margin = await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
        quoteToken.address,
        alice.address
      );
      expect(margin.d).to.eq(toFullDigit(0));
    });
    ...
    it("open position - one long and two shorts", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(20), toDecimal(5), toDecimal(4.17));
      let pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq("33333333333333333333");
      expect(pos.margin.d).to.eq(toFullDigitStr(60));
      expect(
        (
          await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
            quoteToken.address,
      ...
      ).to.eq(toFullDigitStr(60));

      // create position 3 - short
      // AMM after: 1000 : 100
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(10), toDecimal(33.33));
      pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq(0);
      expect(pos.margin.d).to.eq(0);
      expect(
        (
          await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
            quoteToken.address,
    ...
    it("open position - short and two longs", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(20), toDecimal(5), toDecimal(13.88));
      ...
      let pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq("-11111111111111111112");
      expect(pos.margin.d).to.eq(toFullDigitStr(40));
      expect(
        (
          await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
            quoteToken.address,
      ...
      pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq(-1);
      expect(pos.margin.d).to.eq("39999999999999999993");
      expect(
        (
          await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
            quoteToken.address,
    ...
    it("open position - short, long and short", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(20), toDecimal(10), toDecimal(25));
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(150), toDecimal(3), toDecimal(44.9));
      let pos = await clearingHouse.getPosition(amm.address, alice.address);

      // sumSize = -25 + 45 = 20
      expect(pos.size.d).to.eq(toFullDigit(20));

      // sumMargin = sumNotionalSize((20 * 10) - 150 * 3) / leverage(3) = 83.33
      expect(pos.margin.d).to.eq("83333333333333333333");
      expect(
        (
          await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
            quoteToken.address,
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(25), toDecimal(10), toDecimal(19.9));
      pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq(0);
      expect(pos.margin.d).to.eq(0);
      ...
      expect(margin.d).to.eq(0);

      // 1916.666...7 = 2000 - 83.3333...
      expect(await quoteToken.allowance(alice.address, clearingHouse.address)).to.eq("1916666667");
      expect(await quoteToken.balanceOf(alice.address)).to.eq(
    ...
    it("open position - long, short and long", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(150), toDecimal(3), toDecimal(0));

      // sumSize = 20 - 45 = -25
      let pos = await clearingHouse.getPosition(amm.address, alice.address);
      expect(pos.size.d).to.eq(toFullDigit(-25));

      // sumMargin = sumNotionalSize(250 - 450) / leverage(3) = 66.66
      expect(pos.margin.d).to.eq("66666666666666666666");
      expect(
        (
          await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
            quoteToken.address,
    ...
    it("pnl is 0 if no others are trading", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(750), toDecimal(1), toDecimal(0));
    ...
    it("close a safe position", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(2), toDecimal(11.12));
      ...
      await clearingHouse
        .connect(bob)
        .openPosition(amm.address, Side.BUY, toDecimal(10), toDecimal(6), toDecimal(0));
      ...
      expect(position.size.d).to.eq(0);
    ...
    it("close a position which is slightly over maintenanceMarginRatio", async () => {
      ...
      await clearingHouse
        .connect(bob)
        .openPosition(amm.address, Side.SELL, toDecimal(35.08), toDecimal(1), toDecimal(0));
    ...
    it("check PositionChanged event by opening and then closing a position", async () => {
      ...
      const receiptOpen = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(2), toDecimal(11.12));
      await expect(receiptOpen).to.emit(clearingHouse, "PositionChanged").withArgs(
        alice.address,
        amm.address,
        toFullDigit(50),
        toFullDigit(100),
        "-11111111111111111112",
        toFullDigit(1), // notional size 100 * 1% = 1
        "-11111111111111111112",
        "0",
        "0",
        "0",
      ...
      const receiptClose = await clearingHouse
        .connect(alice)
        .closePosition(amm.address, toDecimal(0));
      await expect(receiptClose)
    ...
    it("check PositionChanged event by open 2 opposite side positions with the same size", async () => {
      ...
      const receiptOpen2 = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, amount, toDecimal(1), toDecimal(0));

      await expect(receiptOpen2)
        .to.emit(clearingHouse, "PositionChanged")
        .withArgs(
          alice.address,
          amm.address,
          "49999999999999999993",
          amount.d,
    ...
    it("check PositionChanged event by open a smaller opposite side position", async () => {
      ...
      const receiptOpen2 = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(50), toDecimal(1), toDecimal(0));
    ...
    it("check exchangedPositionSize in PositionChanged event by opening a lager reverse long", async () => {
      ...
      const receipt = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(50), toDecimal(5), toDecimal(0));
      ...
      await expect(receipt)
        ...
        .withArgs(
          alice.address,
          amm.address,
          "29999999999999999998",
          toFullDigit(250),
          "24154589371980676328",
          toFullDigit("0"),
          "13043478260869565216",
          "-8",
          "0",
          "0",
          "0",
          "13224999999999999999",
          "0"
        );
    });
    ...
    it("check exchangedPositionSize in PositionChanged event by opening a lager reverse short", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(50), toDecimal(2), toDecimal(0));

      // got -26.738 position size
      const receipt = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(5), toDecimal(0));
      ...
      await expect(receipt)
        ...
        .withArgs(
          ...
          toFullDigit(30),
          toFullDigit(250),
          "-26737967914438502674",
          "0",
          "-17647058823529411765",
          "-1",
          "0",
          "0",
          "0",
    ...
    it("alice open position, bob open another position, alice reduce position and update margin by closedPnl", async () => {
      // deposit to 2000
      await approve(alice, clearingHouse.address, 2000);
      await approve(bob, clearingHouse.address, 2000);
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(40), toDecimal(10), toDecimal(12.5));

      // now alice has unrealizedPnl 257.14
      // then alice reduce position for 400 quoteToken (equals to 12.5 contract)
      const receipt = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(400), toDecimal(1), toDecimal(12.5));
      ...
      const balance = await clearingHouseViewer.getPersonalBalanceWithFundingPayment(
        quoteToken.address,
        alice.address
      );
      expect(balance.d).eq(toFullDigit(100));
    });
    ...
    it("pnl - unrealized", async () => {
      ...
      await clearingHouse
        .connect(bob)
        .openPosition(amm.address, Side.SELL, toDecimal(100), toDecimal(10), toDecimal(320));
      ...
      const alicePnl = await clearingHouseViewer.getUnrealizedPnl(
        amm.address,
        alice.address,
        PnlCalcOption.SPOT_PRICE
      );
      expect(alicePnl.d).to.eq("-238095238095238095239");
    });
    ...
    it("Force error, open position - not enough balance", async () => {
      ...
      await expect(
        clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(141), toDecimal(10), toDecimal(0))
    ...
    it("Force error, open position - exceed margin ratio", async () => {
      await expect(
        clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(60), toDecimal(21), toDecimal(37.5))
    ...
    it("alice take profit from bob's unrealized under-collateral position, then bob close", async () => {
      ...
      const aliceReceipt = await clearingHouse
        .connect(alice)
        .closePosition(amm.address, toDecimal(0));
      ...
      await expect(aliceReceipt)
        ...
        .withArgs(insuranceFund.address, clearingHouse.address, "74117647");

      expect(await quoteToken.balanceOf(clearingHouse.address)).eq(0);
      expect((await clearingHouse.getPrepaidBadDebt(quoteToken.address)).d).eq(
        "74117647058823529412"
      );
      ...
      expect(BigNumber.from(bobMarginRatio.d).isNegative()).eq(true);
    ...
    it("alice take profit from bob's unrealized under-collateral position, then bob got liquidate", async () => {
      ...
      const bobMarginRatio = await clearingHouse.getMarginRatio(amm.address, bob.address);
      expect(BigNumber.from(bobMarginRatio.d).isNegative()).eq(true);
      await clearingHouse.connect(carol).liquidateWithSlippage(amm.address, bob.address, { d: 0 });
    ...
    it("alice's position has enough margin left, thus won't get liquidated", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(300), toDecimal(2), toDecimal(0));

      // bob opens short position
      await approve(bob, clearingHouse.address, 500);
      await clearingHouse
        .connect(bob)
        .openPosition(amm.address, Side.SELL, toDecimal(500), toDecimal(1), toDecimal(0));
    ...
    it("alice's position got liquidated and not enough margin left for paying liquidation fee", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(150), toDecimal(4), toDecimal(0));
      ...
      const receipt = await clearingHouse
        .connect(carol)
        .liquidateWithSlippage(amm.address, alice.address, { d: 0 });
      ...
      expect(receipt)
        .to.emit(clearingHouse, "PositionChanged")
        .withArgs("-278761061946902654868", "128761061946902654868", "150000000000000000000");
      expect(receipt)
        .to.emit(clearingHouse, "PositionLiquidated")
        .withArgs("8030973451327433628", "8030973451327433628");
    });
    ...
    it("can open reverse position even though position(long) is underwater, as long as the margin ratio will be over maintenance ratio after the action", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(88), toDecimal(2.841), toDecimal(0));
      expect((await clearingHouse.getPosition(amm.address, alice.address)).margin.d).to.eq(
        toFullDigit(88)
      );
      // AMM after 1250 : 80
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(150), toDecimal(1), toDecimal(0));
    ...
    describe("close partial position", () => {
      ...
      it("partially close a long position when closing whole position will over fluctuation limit ", async () => {
        ...
        const receipt = await clearingHouse.connect(alice).closePosition(amm.address, toDecimal(0));
        const pos = await clearingHouse.getPosition(amm.address, alice.address);
        expect(pos.size.d).eq(toFullDigit(15));
        expect(pos.margin.d).eq(toFullDigit(25));
        ...
        await expect(receipt)
          ...
          .withArgs(
            ...
            "0",
            "0",
            "13840830449826989619",
            "0"
          );

        // 5000 - open pos margin (25) + fee (-73.53 * 0.1%)
        expect(await quoteToken.balanceOf(alice.address)).eq("4974926471");
      });
      ...
      it("partially close a short position when closing whole position will over fluctuation limit ", async () => {
        ...
        expect(posAfterOpen.size.d).eq(toFullDigit(-25));
        ...
        const pos = await clearingHouse.getPosition(amm.address, alice.address);
        expect(pos.size.d).eq(toFullDigit(-18.75));
        expect(pos.margin.d).eq(toFullDigit(20));

        await expect(receipt).to.emit(clearingHouse, "PositionChanged").withArgs(
          alice.address,
          amm.address,
          toFullDigit(20),
          "42105263157894736843",
          toFullDigit(6.25), // 25 * 0.25
          "42105263157894736",
          toFullDigit(-18.75), // position size - partial closed position size
          "0",
          "0",
          "0",
          "0",
      ...
      it("should close whole position when partialLiquidationRatio is 1", async () => {
        ...
        const receipt = await clearingHouse.connect(alice).closePosition(amm.address, toDecimal(0));
        const pos = await clearingHouse.getPosition(amm.address, alice.address);
        expect(pos.size.d).eq(toFullDigit(0));
        expect(pos.margin.d).eq(toFullDigit(0));
        expect(await quoteToken.balanceOf(alice.address)).eq(
          toFullDigit(5000, +(await quoteToken.decimals()))
        );
      });
    });
      ...
      describe("valid token amount (>= 10 ^ -6)", () => {
        it("openPosition", async () => {
          const r = await clearingHouse
            .connect(alice)
            .openPosition(amm.address, Side.BUY, toDecimal(1e-6), toDecimal(1), toDecimal(0));
        ...
        it("addMargin", async () => {
          await clearingHouse
            .connect(alice)
            .openPosition(amm.address, Side.BUY, toDecimal(1), toDecimal(1), toDecimal(0));
          const r = await clearingHouse.connect(alice).addMargin(amm.address, toDecimal(1e-6));
        ...
        it("removeMargin", async () => {
          ...
          const r = await clearingHouse.connect(alice).removeMargin(amm.address, toDecimal(1e-6));
          expect(r).to.emit(clearingHouse, "MarginChanged");
        });
      });
      ...
      describe("force error, the token amount is invalid/too small (< 10 ^ -6)", () => {
        it("openPosition", async () => {
          await expect(
            clearingHouse
              .connect(alice)
              .openPosition(amm.address, Side.BUY, toDecimal(0.9e-6), toDecimal(10), toDecimal(0))
        ...
        it("addMargin", async () => {
          ...
          await expect(
            clearingHouse.connect(alice).addMargin(amm.address, toDecimal(0.9e-6))
          ).to.be.revertedWith("invalid token amount");
        });
        ...
        it("removeMargin", async () => {
          ...
          await expect(
            clearingHouse.connect(alice).removeMargin(amm.address, toDecimal(0.9e-6))
          ).to.be.revertedWith("invalid token amount");
        });
      });
    });
  ...
  describe("position upper bound", () => {
    ...
    it("open a long and a smaller short position under limit", async () => {
      ...
      const r = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(110), toDecimal(1), toDecimal(0));
      expect(r).to.emit(clearingHouse, "PositionChanged");

      const r2 = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(1), toDecimal(0));
      expect(r2).to.emit(clearingHouse, "PositionChanged");
    });

    it("open two long positions under limit", async () => {
      const r = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(55), toDecimal(1), toDecimal(0));
      expect(r).to.emit(clearingHouse, "PositionChanged");
    ...
    it("open a short position and a smaller long under limit", async () => {
      ...
      const r = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(90), toDecimal(1), toDecimal(0));
      expect(r).to.emit(clearingHouse, "PositionChanged");
    ...
    it("open two short positions under limit", async () => {
      const r = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(45), toDecimal(1), toDecimal(0));
      expect(r).to.emit(clearingHouse, "PositionChanged");
    ...
    it("change position size upper bound and open positions", async () => {
      ...
      const r2 = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(16), toDecimal(10), toDecimal(0));
      expect(r2).to.emit(clearingHouse, "PositionChanged");
    });
    ...
    it("force error, open a long position and over the limit", async () => {
      ...
      const r = await expect(
        clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(120), toDecimal(1), toDecimal(0))
    ...
    it("force error, open long positions and over the limit", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.BUY, toDecimal(60), toDecimal(1), toDecimal(0));
    ...
    it("force error, open a short position and over the limit", async () => {
      ...
      const r = await expect(
        clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(95), toDecimal(1), toDecimal(0))
      ).to.be.revertedWith("hit position size upper bound");
    });
    ...
    it("force error, open a short and a larger reverse long and over the limit", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(9), toDecimal(10), toDecimal(0));

      // position size would be 10.7, revert
      const r = await expect(
        clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(21), toDecimal(10), toDecimal(0))
      ).to.be.revertedWith("hit position size upper bound");
    });
    ...
    //         { from: alice }
    //       );
    //       await expect(r).to.emit(clearingHouse, "PositionChanged");
    //     });
  ...
  describe("fee calculation", () => {
    ...
    it("open short position twice when total fee is 10%", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(2), toDecimal(11.2));
      const aliceBalance1 = await quoteToken.balanceOf(alice.address);

      const receipt = await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(2), toDecimal(139));
      ...
      await expect(receipt)
        ...
        .withArgs(
          ...
          toFullDigit(100),
          "-13888888888888888889",
          toFullDigit(10),
          "-25000000000000000001",
          "0",
          "21951219512195121945",
          "0",
          "0",
          "6399999999999999999",
          "0"
    ...
    it("open position and close manually by opening reverse position(short then long) when fee is 10%", async () => {
      ...
      await clearingHouse
        .connect(alice)
        .openPosition(amm.address, Side.SELL, toDecimal(300), toDecimal(2), toDecimal(0));
  ...
  describe("traded with 10% fee amm, check size, margin and openNotional", () => {
    ...
    describe("open position", () => {
      it("open long position", async () => {
        ...
        const position = await clearingHouse.getPosition(amm.address, alice.address);
        expect(position.size.d).to.eq(toFullDigit(37.5));
        expect(position.openNotional.d).to.eq(toFullDigit(600));
        expect(position.margin.d).to.eq(toFullDigit(60));
      ...
      it("open short position", async () => {
        ...
        // transferred margin = margin + fee = 60 + (60 * 10 * 10%) = 120
        const position = await clearingHouse.getPosition(amm.address, alice.address);
        expect(position.size.d).to.eq(toFullDigit(-150));
        expect(position.openNotional.d).to.eq(toFullDigit(600));
        expect(position.margin.d).to.eq(toFullDigit(60));
    ...
    describe("increase position", () => {
      it("open long position, price remains, then long again", async () => {
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(175), toDecimal(2), toDecimal(17.5));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);

        // transferred margin = margin + fee = 175 + (175 * 2 * 10%) = 210
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(-210, +(await quoteToken.decimals()))
        );
        ...
        expect(position.margin.d).to.eq(toFullDigit(200));
      ...
      it("open long position, price up, then long again", async () => {
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.BUY, toDecimal(35), toDecimal(10), toDecimal(17.5));
        ...
        ).eq("137878787878787878787");

        // alice opens long position with 200 margin, 2x leverage
        // (1600 + 400) * (62.5 + baseAssetDelta) = 100k, baseAssetDelta = 12.5
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(200), toDecimal(2), toDecimal(12.5));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);

        // transferred margin = margin + fee = 200 + (200 * 2 * 10%) = 240
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(-240, +(await quoteToken.decimals()))
        );

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = 20 + 12.5 = 32.5
        expect(position.size.d).to.eq(toFullDigit(32.5));
        // open notional = 250 + 400 = 650
        expect(position.openNotional.d).to.eq(toFullDigit(650));
        // total position margin = 25 + 200 = 225
        expect(position.margin.d).to.eq(toFullDigit(225));
      });
      ...
      it("open long position, price down, then long again", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(125), toDecimal(2), toDecimal(20));
        const aliceBalance1 = await quoteToken.balanceOf(alice.address);
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.SELL, toDecimal(125), toDecimal(2), toDecimal(20));
        ...
        ).eq("-83333333333333333334");

        // alice opens long position with 50 margin, 5x leverage
        // (1000 + 250) * (100 + baseAssetDelta) = 100k, baseAssetDelta = -20
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(50), toDecimal(5), toDecimal(20));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);

        // transferred margin = margin + fee = 50 + (50 * 5 * 10%) = 75
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(-75, +(await quoteToken.decimals()))
        );

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = 20 + 20 = 40
        expect(position.size.d).to.eq(toFullDigit(40));
        // open notional = 250 + 250 = 500
        expect(position.openNotional.d).to.eq(toFullDigit(500));
        // total position margin = 125 + 50 = 175
        expect(position.margin.d).to.eq(toFullDigit(175));
      });
      ...
      it("open short, price remains, then short again", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(100), toDecimal(2), toDecimal(25));
        const aliceBalance1 = await quoteToken.balanceOf(alice.address);
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(8), toDecimal(125));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);

        // then transferred margin = margin + fee = 50 + (50 * 8 * 10%) = 90
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(-90, +(await quoteToken.decimals()))
        );
        ...
        expect(position.margin.d).to.eq(toFullDigit(150));
        // pnl = 0 because no other trader
        expect(
          (
            await clearingHouseViewer.getUnrealizedPnl(
      ...
      it("open short, price down, then short again", async () => {
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.SELL, toDecimal(150), toDecimal(2), toDecimal(75));
        ...
        ).eq("128571428571428571428");

        // alice opens short position with 100 margin, 3x leverage
        // (500 - 300) * (200 + baseAssetDelta) = 100k, baseAssetDelta = 300
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(100), toDecimal(3), toDecimal(300));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);

        // transferred margin = margin + fee = 100 + (100 * 3 * 10%) = 130
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(-130, +(await quoteToken.decimals()))
        );

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = -25 - 300 = -325
        expect(position.size.d).to.eq(toFullDigit(-325));
      ...
      it("open short, price up, then short again", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(200), toDecimal(1), toDecimal(25));
        const aliceBalance1 = await quoteToken.balanceOf(alice.address);
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.BUY, toDecimal(200), toDecimal(1), toDecimal(25));
        ...
        ).eq("-133333333333333333334");

        // alice opens short position with 50 margin, 4x leverage
        // (1000 - 200) * (100 + baseAssetDelta) = 100k, baseAssetDelta = 25
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(50), toDecimal(4), toDecimal(25));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);

        // then transferred margin = margin + fee = 50 + (50 * 4 * 10%) = 70
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(-70, +(await quoteToken.decimals()))
        );

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = -25 - 25 = -50
        expect(position.size.d).to.eq(toFullDigit(-50));
        // open notional = 200 + 200 = 400
        expect(position.openNotional.d).to.eq(toFullDigit(400));
        // total position margin = oldMargin + newMargin + realizedPnl = 200 + 50 + 0 = 250
        expect(position.margin.d).to.eq(toFullDigit(250));
      });
    });
    ...
    describe("reduce position", () => {
      it("open long position, price remains, then reduce position", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(350), toDecimal(1), toDecimal(17.5));

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = 37.5 - 17.5 = 20
        expect(position.size.d).to.eq(toFullDigit(20));
        // openNotional = originalPositionNotional - reducedPositionNotional = 600 - 350 = 250
        expect(position.openNotional.d).to.eq(toFullDigit(250));
      ...
      it("open short position, price remains, then reduce position", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(400), toDecimal(1), toDecimal(125));

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = -150 + 125 = -25
        expect(position.size.d).to.eq(toFullDigit(-25));
        // openNotional = positionNotional(200) - unrealizedPnl(0) = 200
        expect(position.openNotional.d).to.eq(toFullDigit(200));
      ...
      it("open long position, price up, then reduce position", async () => {
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.BUY, toDecimal(400), toDecimal(1), toDecimal(12.5));
        ...
        expect(position.openNotional.d).to.eq("285714285714285714285");
        // total position margin = 60 + realizedPnl = 60 + 85.61 = 145.61
        expect(position.margin.d).to.eq("145714285714285714285");
      });
      ...
      it("open long position, price down, then reduce position", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(500), toDecimal(2), toDecimal(50));
        ...
        expect(position.openNotional.d).to.eq("548888888888888888889");
        // total position margin = oldMargin + realizedPnl = 500 - 101.1111111111 = 398.888888889
        expect(position.margin.d).to.eq("398888888888888888889");
      });
      ...
      it("open short position, price up, then reduce position", async () => {
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.BUY, toDecimal(50), toDecimal(1), toDecimal(7.35));
        ...
        expect(position.openNotional.d).to.eq("70728291316526610643");
        // total position margin = margin + realizedPnl = 100 - 20.7282913155 = 79.2717086845
        expect(position.margin.d).to.eq("79271708683473389357");
      });
      ...
      it("open short position, price down, then reduce position", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(250), toDecimal(2), toDecimal(100));

        // bob opens short position with 100 margin, 1x leverage. price down
        // (500 - 100) * (200 + baseAssetDelta) = 100k, baseAssetDelta = 50
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.SELL, toDecimal(100), toDecimal(1), toDecimal(50));
        ...
        ).eq("233333333333333333333");

        // alice reduce position in 100 quoteAsset amount
        // (400 + 100) * (250 + baseAssetDelta) = 100k, baseAssetDelta = -50
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(100), toDecimal(1), toDecimal(50));
        ...
        expect(position.openNotional.d).to.eq("283333333333333333334");
        // total position margin = margin + realizedPnl = 250 + 116.6666666666 = 366.6666666666
        expect(position.margin.d).to.eq("366666666666666666666");
      });
    });
    ...
    describe("manually close position", () => {
      it("open long position, price remains, then close entire position manually", async () => {
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(250), toDecimal(1), toDecimal(20));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);
        ...
        await expect(receipt)
          ...
          .withArgs(
            clearingHouse.address,
            alice.address,
            toFullDigit(50, +(await quoteToken.decimals()))
          );

        // transferred margin = closedMargin - fee + realizedPnl = 50 - 25 + 0 = 25
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(25, +(await quoteToken.decimals()))
        );

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        expect(position.size.d).to.eq(toFullDigit(0));
        expect(position.openNotional.d).to.eq(toFullDigit(0));
        expect(position.margin.d).to.eq(toFullDigit(0));
      });
      ...
      it("open long position, price up, then close entire position manually", async () => {
        ...
        expect(currentPositionValue[1].d).eq("137878787878787878787");
      ...
      it("open long position, price down, then close entire position manually", async () => {
        ...
        expect(currentPositionValue[1].d).eq("-288888888888888888889");
      ...
      it("open short position, price up, then close entire position manually", async () => {
        ...
        expect(currentPositionValue[1].d).eq("-29365079365079365079");
      ...
      it("open short position, price down, then close entire position manually", async () => {
        ...
        expect(currentPositionValue[1].d).eq("233333333333333333333");
    ...
    describe("opens a position, then opens an larger position in reversed direction", () => {
      it("open long position, price remains, then close entire position by opening another larger short", async () => {
        ...
        expect(position.openNotional.d).to.eq(toFullDigit(200));
        // newRequireMargin
        expect(position.margin.d).to.eq(toFullDigit(20));
        expect(
          (
            await clearingHouseViewer.getUnrealizedPnl(
              amm.address,
      ...
      it("open short position, price remains, then close entire position by opening another larger long", async () => {
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(90), toDecimal(5), toDecimal(45));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);
        ...
        expect(position.margin.d).to.eq(toFullDigit(50));
      ...
      it("open long position, price up, then close entire position by opening another larger short", async () => {
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(100), toDecimal(8), toDecimal(62.51));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);
        ...
        expect(position.size.d).to.eq("-42500000000000000001");
        ...
        expect(position.openNotional.d).to.eq("412121212121212121213");
        // requiredNewMargin = remainPositionNotional/newLeverage = 412.12/8 = 51.515
      ...
      it("open long position, price down, then close entire position by opening another larger short", async () => {
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(60), toDecimal(10), toDecimal(1450));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);
        ...
        expect(position.size.d).to.eq("-130000000000000000001");
        // remain unrealizedPnl = unrealizedPnl - realizedPnl = 0
        expect(
          (
            await clearingHouseViewer.getUnrealizedPnl(
        ...
        expect(position.openNotional.d).to.eq("433333333333333333334");
        // total position margin = 433.33 / 10
        expect(position.margin.d).to.eq("43333333333333333333");
      });
      ...
      it("open short position, price up, then close entire position by opening another larger long", async () => {
        ...
        await clearingHouse
          .connect(bob)
          .openPosition(amm.address, Side.BUY, toDecimal(50), toDecimal(4), toDecimal(7.349));
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(60), toDecimal(10), toDecimal(37.49));

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = 37.5 - 25 = 12.5 - 1 wei
        expect(position.size.d).to.eq("12499999999999999999");
        ...
        expect(position.openNotional.d).to.eq("266666666666666666666");
        // margin is positionNotional / leverage = 26.66
        expect(position.margin.d).to.eq("26666666666666666666");
      });
      ...
      it("open short position, price down, then close entire position by opening another larger long", async () => {
        ...
        await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.SELL, toDecimal(500), toDecimal(1), toDecimal(100));
        const aliceBalance1 = await quoteToken.balanceOf(alice.address);
        ...
        const receipt = await clearingHouse
          .connect(alice)
          .openPosition(amm.address, Side.BUY, toDecimal(60), toDecimal(10), toDecimal(149.99));
        const aliceBalance2 = await quoteToken.balanceOf(alice.address);
        ...
        expect(aliceBalance2.sub(aliceBalance1)).eq(
          toFullDigit(640, +(await quoteToken.decimals()))
        );

        const position = await clearingHouse.getPosition(amm.address, alice.address);
        // total position size = 150 - 100 = 50 - 1 wei
        expect(position.size.d).to.eq("49999999999999999999");
        ...
        expect(position.openNotional.d).to.eq("333333333333333333333");
        // total position margin = 333.33 / 10
        expect(position.margin.d).to.eq("33333333333333333333");
      });
    });
  });
});
