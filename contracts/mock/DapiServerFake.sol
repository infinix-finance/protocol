...
contract DapiServerFake {
    ...
    function mockDapiName(bytes32 dapiName, bytes32 dataFeedId) external {
        ...
        dapiNameHashToDataFeedId[keccak256(abi.encodePacked(dapiName))] = dataFeedId;
    }
    ...
    // Reads a data feed using the dAPI name that is mapped to a data feed.
    // Omits the timestamp.
    function readDataFeedValueWithDapiName(bytes32 dapiName) external view returns (int224 value) {
        bytes32 dapiNameHash = keccak256(abi.encodePacked(dapiName));
        require(allowedToRead, "Sender cannot read");
        DataFeed storage dataFeed = dataFeeds[dapiNameHashToDataFeedId[dapiNameHash]];
